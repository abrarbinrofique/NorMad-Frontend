...
async function BookTravel(event) {
    ...
    try {
        const response = await fetch(`https://normad-bakend.vercel.app/event/list/${k}/addpeople/`, {
            method: "POST",
            headers: {
...
async function ShareTravel(event) {
   ...
   if(vacancy===peoplelimit)
    ...
    console.log(formData,k)
    try {
        const response = await fetch(`https://normad-bakend.vercel.app/sharetravel/join/`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
            },
